// Closure basically means, that a function bind together along with its lexical environment or lexical scope is known as Closure
...
/** Disadvantages of closures: Over consumption of memory upon using closures
    whenever a closure is formed, it consumes memory and those closed over variables are-not garbage collected
    cont. If not handled properly it can also lead to memory leaks, because over the time memory is accumulated
    cont. and it can also freeze the browser if not handled properly
*/

/** What is meant by Garbage collector? Garbage collector is a program in the browser (or) the job of javascript engine which can free up the unutilised memory
    Javascript is a high level language right generally in low level languages like C or C++, 
    cont. we generally it is up to developer wish how we adds memory, how we access to the memory, 
    cont. how we allocate memory or deallocate memory, it's up to developers wish 
    cont. but in high level programming like javascript most of the work is done by javascript engine, 
    cont. so there is a garbage collector in javascript, this garbage collector whenever there are some unused variables, 
    cont. it just takes it out of memory it just frees up the memory
 */
/** How are closures and garbage collector related to each other? 
    cont. So when we come over the closure and the variable attached to it, 
    cont. we can't able to free up these variables because we can use the return function of closure somewhere in program, 
    cont. and these variables are not garbage collected, but some of the modern browsers like chrome 
    cont. which uses v8 javascript engine has “smart-garbage-collection-mechanism” where it like somehow 
    cont. it finds out that these variable not used, they smartly-collects these garbage variables
 */
...
// The function in setTimeOut remembers “i value” wherever it goes since it forms a closure with the main function closureTimeout()
